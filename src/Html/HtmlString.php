...
use Deepayan\LaraSpeak\Embera\Adapter;
...
class HtmlString implements Htmlable
{
    ...
    {
        if (config('laraspeak.oembed.enabled')) {
            $parsedUrl = parse_url(config('laraspeak.oembed.url'));
            $this->driver->addProvider(
                $parsedUrl['host'],
                Adapter::class,
                [
                    'api_key' => config('laraspeak.oembed.key'),
                ]
            );
        }
